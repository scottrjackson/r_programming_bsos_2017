...
color.table2 <- data.frame(Species = c("setosa", "virginica", "versicolor", "junos"), Color = c("red", "blue", "violet", "pink"))
color.table3 <- data.frame(Species = c("virginica", "versicolor"), Color = c("blue", "violet"))

iris.merge1 <- merge(iris, color.table)
iris.merge1.all <- merge(iris, color.table, all = TRUE)
iris.merge2 <- merge(iris, color.table2)
iris.merge2.all <- merge(iris, color.table2, all = TRUE)
iris.merge3 <- merge(iris, color.table3)
iris.merge3.all <- merge(iris, color.table3, all = TRUE)

head(iris.merge1)
head(iris.merge1.all)
nrow(iris)
nrow(iris.merge1)
nrow(iris.merge1.all)

head(iris.merge2)
head(iris.merge2.all)
nrow(iris)
nrow(iris.merge2)
nrow(iris.merge2.all)
tail(iris.merge2.all)

head(iris.merge3)
head(iris.merge3.all)
nrow(iris)
nrow(iris.merge3)
nrow(iris.merge3.all)
summary(iris.merge3.all)
...
```{r}
myris2 <- iris
myris2$logSW <- log(myris2$Sepal.Width)
myris2$SepalSquare <- myris2$Sepal.Width * myris2$Sepal.Length
median.SW <- median(myris2$Sepal.Width)
myris2$SW.split[myris2$Sepal.Width < median.SW] <- "low"
myris2$SW.split[myris2$Sepal.Width >= median.SW] <- "high"
myris2$SW.split <- factor(myris2$SW.split, levels = c("low", "high"))
...
head(myris3.long)
summary(myris3.long)
summary(as.factor(myris3.long$measurement))
